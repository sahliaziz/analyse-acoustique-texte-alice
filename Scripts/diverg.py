...
class ModelLongTerm(object):
    ...
    def miseAJour(self, echantillon):
        ...
        for n in xrange(ik + 1):
            ...
            self.variance_f[n + 1] = self.variance_f[n + 1] + oubli * (
                self.et[n + 1] ** 2 - self.variance_f[n + 1]
            )
            self.variance_b[n + 1] = self.variance_b[n + 1] + oubli * (
                self.ft[n + 1] ** 2 - self.variance_b[n + 1]
            )
...
class ModelCourtTrerm(object):
    ...
    def estimModel(self):
        ...
        self.variance_erreur_residuelle = (
            self.coef_autocorr[0] + self.coef_autocorr[1] * coef_reflexion[0]
        )
        ...
        if self.variance_erreur_residuelle > 0:
            ...
            for i in range(self.ordre + 1):
                self.erreur_residuelle = (
                    self.erreur_residuelle + self.AI[i] * self.buff[self.N - i - 1]
                )
    ...
    def miseAJour(self, echantillon):
        ...
        for tau in xrange(1, self.ordre + 1):
            self.coef_autocorr[tau] = (
                ...
                + self.buff[self.N - tau - 1] * self.buff[self.N - 1]
            )
        self.coef_autocorr[0] = (
            self.coef_autocorr[0]
            - self.dernier_echantillon**2
            + self.buff[self.N - 1] ** 2
        )
        self.estimModel()
...
class LineBuilder:
    ...
    def __init__(self, trace, signal, Wlen=256, pas=25, Nfft=1024):
        ...
        self.axarr[0].imshow(
            spectro,
            origin="lower",
            extent=[timeLine[0], timeLine[-1], 0, fe / 2],
            aspect="auto",
        )
