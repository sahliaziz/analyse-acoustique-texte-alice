...
tab_to_csv = [['sujet', 'genre', 'repetition',
               'phoneme', 'cog', 'sd', 'skew', 'kurt']]
for i in range(1, len(df.axes[0])):
    if df.iloc[i][2] != '--undefined--' and df.iloc[i][3] != '--undefined--' and df.iloc[i][4] != '--undefined--' and \
            df.iloc[i][5] != '--undefined--':
        temporary_tab = []
...
print("Time elapsed during mesures_acoustiques_consonnes.py =",
