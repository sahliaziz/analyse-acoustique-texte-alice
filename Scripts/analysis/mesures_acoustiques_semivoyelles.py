...
tab = [["Sujet", "Genre", "Repetition", "phoneme", "f1_slope", "f2_slope", "f3_slope"]]
...
for file in range(len(list_files)):
    ...
    for lines in range(1, len(df.axes[0])):
        temporary_list = [
            ...
            rep,
            df.iloc[lines][0],
            float(df.iloc[lines][9]),
            float(df.iloc[lines][10]),
            float(df.iloc[lines][11]),
        ]
...
df_mean.to_csv(
    "./Analyzed_results/mesures_acoustiques_semivoyelles.csv", index=False, header=False
)
