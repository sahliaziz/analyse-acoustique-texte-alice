...
def vowel_counter(tg_content: str) -> int:
    ...
    """
    sampa_vowels = {"@", "a~", "E", "9~", "e~", "O", "a", "e", "i", "o", "o~", "u", "y"}
    ipa_vowels = {"a", "e", "i", "o", "u", "y", "ø", "œ", "ɑ̃", "ɔ", "ɔ̃", "ə", "ɛ", "ɛ̃", "œ̃"}
    
    tiers = tg_content.split("item [")
    phones_content = None
    is_ipa = False
    ...
    for tier in tiers[1:]:
        ...
        if name_match:
            ...
            if name == "phones":
                ...
                phones_content = tier
                is_ipa = False
            elif name == "MAUS" and phones_content is None:
                phones_content = tier
                is_ipa = False
                
    if phones_content is None:
        tier3_content = tg_content.partition("item [3]")[0] + "item [3]"
        labels = re.findall(r'text\s*=\s*"(.*?)"', tier3_content)
        vowels_set = sampa_vowels
    else:
        labels = re.findall(r'text\s*=\s*"(.*?)"', phones_content)
        vowels_set = ipa_vowels if is_ipa else sampa_vowels
        
    return sum(1 for label in labels if label.strip() in vowels_set)
