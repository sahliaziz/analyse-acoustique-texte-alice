...
class ModelLongTerm(object):
    ...
    def __init__(self, ordre, echantillon):
        ...
        self.variance_f[0] = self.variance_f[0] + \
            oubli*(echantillon**2-self.variance_f[0])
    ...
    def miseAJour(self, echantillon):
        ...
        for n in xrange(ik+1):
            ...
            if (self.variance_f[n] + self.variance_b[n]) == 0:
                ...
                knplus1 = 2*self.cor[n] / \
                    (self.variance_f[n] + self.variance_b[n])

            self.et[n+1] = self.et[n]-knplus1*self.ftm1[n]
            self.ft[n+1] = self.ftm1[n]-knplus1*self.et[n]

            self.variance_f[n+1] = self.variance_f[n+1] + \
                oubli*(self.et[n+1]**2-self.variance_f[n+1])
            self.variance_b[n+1] = self.variance_b[n+1] + \
                oubli*(self.ft[n+1]**2-self.variance_b[n+1])
...
class ModelCourtTrerm(object):
    ...
    def __init__(self, ordre, buff):
        ...
        for tau in xrange(self.ordre+1):
            for i in xrange(self.N-tau):
                self.coef_autocorr[tau] = self.coef_autocorr[tau] + \
                    buff[i]*buff[i+tau-1]
        self.estimModel()
    ...
    def estimModel(self):
        ...
        self.variance_erreur_residuelle = self.coef_autocorr[0] + \
            self.coef_autocorr[1]*coef_reflexion[0]
            ...
            while i_ordre < self.ordre and self.variance_erreur_residuelle > 0:
                ...
                if self.variance_erreur_residuelle > 0:
                    ...
                    for i in xrange(1, MH):
                        ...
                        self.AI[IB] = self.AI[IB] + \
                            coef_reflexion[i_ordre]*self.AI[i]
                        self.AI[i] = tmp
    ...
    def miseAJour(self, echantillon):
        ...
        for tau in xrange(1, self.ordre+1):
            self.coef_autocorr[tau] = self.coef_autocorr[tau]-self.dernier_echantillon * \
                self.buff[tau-1]+self.buff[self.N-tau-1]*self.buff[self.N-1]
        self.coef_autocorr[0] = self.coef_autocorr[0] - \
            self.dernier_echantillon**2+self.buff[self.N-1]**2
        self.estimModel()
...
def segment(data, fe, ordre=2, Lmin=0.02, lamb=40.0, biais=-0.2, with_backward=True, seuil_vois=None, withTrace=False):
    ...
    while t < long_signal-1:
        ...
        if withTrace:
            evnt = {'Wn': dynaWn, 'timeLine': tWn,
                    't_rupt': t_rupt, 'Wn_max': maxi[0]}
        ...
        if t_rupt > -1:
            ...
            if with_backward:
                ...
                if len(bdata) > 0:
                    ...
                    front, btrace = segment(bdata, fe, ordre, float(
                        Lmin)/fe, lamb, biais, with_backward=False, seuil_vois=seuil_vois, withTrace=withTrace)
...
class LineBuilder:
    ...
    def __init__(self, trace, signal, Wlen=256, pas=25, Nfft=1024):
        ...
        self.axarr[0].imshow(spectro, origin='lower', extent=[
                             timeLine[0], timeLine[-1], 0, fe/2], aspect='auto')
...
if __name__ == '__main__':
    ...
    frontieres, trace = segment(
        data, fe, ordre=order, with_backward=True, withTrace=VERBOSE, seuil_vois=seuil_vois)

    if VERBOSE:
        print 'Ordre %d : %d Frontieres (%.4f sec)' % (
            ordre, len(frontieres), time.time()-st)
